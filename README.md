...
|--------------|------------|------------------|
| **`query_key_pair!`**<br>Finds the path to a key-value pair in a JSON-like structure.<br>**Example:**<br>```rust<br>query_key_pair!(mydict, "key" => "value");<br>``` | **`print_pretty!`**<br>Pretty-prints a JSON-like structure in a formatted way.<br>**Example:**<br>```rust<br>print_pretty!(mydict);<br>``` | **`ensure_exist_with_schema!`**<br>Checks if a file exists and its schema; creates it with the specified JSON if not.<br>**Example:**<br>```rust<br>ensure_exist_with_schema!("test.db", serde_json::json!({"key": "value"}));<br>``` |
| **`exist_same_level!`**<br>Checks if all provided key-value pairs exist at the same level in a JSON-like structure.<br>**Example:**<br>```rust<br>exist_same_level!(mydict, "destination_ip" => destination_ip, "packet_type" => packet_type);<br>``` | **`root_append!`**<br>Appends a key-value pair to the root of a JSON-like structure.<br>**Example:**<br>```rust<br>root_append!(mydict, "new_key" => "new_value");<br>``` | **`ensure_exist!`**<br>Ensures that a file exists by creating it if it doesn't.<br>**Example:**<br>```rust<br>ensure_exist!("data.db");<br>``` |
| **`query_value!`**<br>Searches for a specific value in a JSON-like structure and returns its path.<br>**Example:**<br>```rust<br>query_value!(mydict, "value");<br>``` | **`set!`**<br>Sets a value in a JSON-like structure by its path.<br>**Example:**<br>```rust<br>set!(mydict, "key.subkey", "new_value");<br>``` | **`load!`**<br>Loads a JSON-like structure from a file.<br>**Example:**<br>```rust<br>let mydict = load!("data.json");<br>``` |
| **`exists!`**<br>Checks if a specific key or value exists in a JSON-like structure.<br>**Example:**<br>```rust<br>exists!(mydict, "key");<br>``` | **`append!`**<br>Appends a value to an array within a JSON-like structure.<br>**Example:**<br>```rust<br>append!(mydict, "key.array", "new_value");<br>``` | **`save!`**<br>Saves a JSON-like structure to a file.<br>**Example:**<br>```rust<br>save!(mydict, "data.json");<br>``` |
| **`get!`**<br>Retrieves a value from a JSON-like structure by its path.<br>**Example:**<br>```rust<br>let value = get!(mydict, "key.subkey");<br>``` | **`delete!`**<br>Deletes a key or value from a JSON-like structure by its path.<br>**Example:**<br>```rust<br>delete!(mydict, "key.subkey");<br>``` |  |
